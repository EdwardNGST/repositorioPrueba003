...
namespace ITCG_FERRETERIA.c_admin.c_delete.c_inhabilitarProveedor
{
    class clsInhabilitarProveedor
    {
        ...
        public void inhabilitarProveedor(string id)
        {
            int idI = Convert.ToInt32(id);
            if (verificarProveedor(idI) != "")
            {
                storeProcedureInhabilitar(idI);
                MessageBox.Show("Proveedor Inhabilitado Exitosamente");
            }
            else
            {
        ...
        public void storeProcedureInhabilitar(int id)
        {
            ...
            comando = new MySqlCommand("inhabilitarProveedor", conector);
            comando.CommandType = CommandType.StoredProcedure;

            comando.Parameters.AddWithValue("@codigoproveedor", id);
            ...
            conector.Close();
            MessageBox.Show("Actualizacion Finalizada Correctamente!");
        }
    }
}